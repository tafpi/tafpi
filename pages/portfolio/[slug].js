...
const PortfolioItem = ({item}) => {
	if (!item) return null;
	const {
		title, description, content, featuredImage: imageSrc, images, url: externalLink, category: categories, tools
	} = item;
	return (
		<>
			<Head>
				...
				<div className={'bg-gray-100 pb-8 lg:pb-16'}>
					<div className={'container max-w-screen-lg mx-auto px-4'}>
						...
						<div className={'flex flex-col gap-4 lg:gap-8'}>
							...
							{images?.map(image => {
								return <Image key={image.asset._id} src={image.asset.url} width={'900'} height={'900'}
															className={'border'}
															alt={''}/>
							})}
						</div>
