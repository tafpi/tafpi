...
const Archive = ({slug, type, path, allLabel, title, tags, items}) => {
	return (
		<div className={'bg-gray-100'} style={{minHeight: 'calc(100vh - 150px)'}}>
					...
					<div className="py-8 md:py-14">
						<div className="grid gap-6 md:gap-12">
							{items.map(item => {
								...
								return (
									<PortfolioItem
										key={id} slug={slug} title={title} description={description} imageSrc={featuredImage}
										externalLink={url} categories={categories} tools={tools}
									/>
								)
							})}
						</div>
